...
/** 自発カロート生成プロンプト */
export function buildAutoPostPrompt(tlContext: string, recentPosts: string[], timeContext: string, cloneContext?: string): string {
  const myHistory = recentPosts.length > 0
    ? `【あなたの直近の投稿履歴】\n${recentPosts.map(p => `- ${p}`).join('\n')}\n\n`
    : '';

  const cloneInstruction = cloneContext
    ? `※【重要】以下はあなたが徹底的に真似るべき人物の直近の投稿リストです。\nこの人物の「言葉遣い、口癖、テンション、絵文字・記号の使い方の癖、文末の表現」などの【話し方やスタイル】を極限まで忠実に模倣してください。\n\n⚠️注意: この人物がしている「特定の行動・予定・出来事（例：テレビを見る、どこかへ行くなど）」の内容自体は絶対に真似しないでください。話題や内容はあなた独自のものである必要があり、あくまで『言葉のトーン（ガワ）』だけを完全に憑依させてください。\n\n【模倣対象の参考投稿リスト】\n${cloneContext}\n\n`
    : '';
...
${timeContext}${myHistory}${cloneInstruction}この空気に紛れ込むような、あなたの短い独り言を作成してください。日常のつぶやき、疑問形、クスッと笑えるネタなど、毎回パターンを変えてバリエーション豊かにしてください。`;
}
...
${timeContext}${context}${cloneInstruction}相手の引用コメント: ${content}`;
  }

  return `以下のメッセージに返信して。画像がある場合はそれも見て。返信不要な場合のみ「SKIP」と出力。

${timeContext}${context}${cloneInstruction}メッセージ: ${content}`;
}
...
export function getTimeContext(): string {
  ...
  return `【参考情報】現在時刻: ${jstNow.getUTCMonth() + 1}月${jstNow.getUTCDate()}日（${weekdays[jstNow.getUTCDay()]}）${jstNow.getUTCHours()}時${jstNow.getUTCMinutes()}分\n`;
}
